...
* Това означава, че е написана на C и е оптимизирана за конкурентно писане и четене, а вътршно съхранява данни, които са MUTABLE. |
...
* Сега едно извикване до RequestsPerUser.get/1 може да отнеме дадено време... |
...
---
### Нуждата от ETS
* Така имаме bottleneck - много процеси използват един процес за пазене и промяна на дадено състояние. |
* Дори една програма да използва множество нишки/процеси за да постигне конкурентно писане и четене, ако имаме една нишка/процес, до която всички други опират, все едно имаме последователна програма. |
* И тук идва въпросът как да имплементираме споделено състояние между процеси в BEAM с конкурентен достъп. |
* Отговорът е ETS. |
...
---
### Какво е ETS?
* ETS не е имплементирана на Erlang и не изградена от Erlang процеси и persistent струтурите от данни като Map или List. |
...
```elixir
:ets.insert(:users, {"meddle", 34, "Sofia", [:elixir, :programming, :gaming, :meddling]})
#=> ** (ArgumentError) argument error
#=>     (stdlib) :ets.insert(:users, {"meddle", 34, "Sofia", [:elixir, :programming, :gaming, :meddling]})
```
...
```elixir
:ets.insert(table, {"meddle", 33, "Sofia", [:elixir, :programming, :gaming, :meddling]}) 
#=> true
...
:ets.insert(
  :people,
  [
    {"Петър", "Петров", "pe60", 55, ["Еврофутбол", "мачове", "ракийка и салатка", "Цеца"]},
    {"Милена", "Стоева", "milen4it0", 23, ["социални мрежи", "кафенца", "клубчета", "дрешки"]}
  ]
)
#=> true
...
:ets.insert_new(
  ...
  [
    {"Милена", "Стоева", "milen4it0", 23, ["социални мрежи", "кафенца", "клубчета", "дрешки"]},
    {"Николай", "Цветинов", "meddle", 33, ["гледане в точка", "elixir", "the binding of isaac", "black sabbath"]}
  ]
)
#=> false
...
---
Има много начини да четем от ETS таблици. Най-простият начин е да търсим по ключ:
...
* Result : Как да изглежда резултата. Може да са някакви трансформации.
...
* При :ordered_set таблици редът е по ключовете, които са сортирани, ако таблицата е друг тип редът заивиси от това как се пазят записите вътрешно. |
...
* Лесен начин е да се иползва :ets.insert/2 за вече съществуващ ключ, което ще презапише реда. |
...
Има специализирани функции update_counter за атомарно увеличаване или намаляване ан брояч, пазен в таблицата.
...
---
### Наследници
* Ако процесът owner спре да същестува, ETS таблицата, притежавана от него бива зачистена от паметта. |
...
* Вместо това, процесът-наследник получава съобщение от типа {:"ETS-TRANSFER", <име или референция към таблицата>, <pid на предишния собственик>, <информация, зададена при определянето на наследник>}. |
...
* Това означава, че всичко освен ключа на даден запис ще е компресирано, което ще оптимизира паметта използвана от таблицата. |
* Цената на това е, че четене на цели редове ще стане по-бавно. |
...
* Понякога ETS таблица може да се използва вместо Map или List за оптимизация, макар че не винаги това ще е най-бързото решение. |
* ETS може да бъде използвана за регистриране на познати процеси. Всъщност в Elixir има такава функционалност имплементирана чрез модула Registry. |
